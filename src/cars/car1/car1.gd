...
func get_input(delta : float):
	...
	if player_input.y > 0.01:
		...
		player_acceleration = 0.0
		player_braking = 0.0
	audio.pitch_scale = 1.0 + abs(driving_wheels[0].get_rpm() / max_wheel_rpm)
...
func _change_audio(trackName: String) -> void:
	var currentTrack = audio["parameters/switch_to_clip"]
	if trackName != currentTrack:
		audio["parameters/switch_to_clip"] = trackName
...
func _on_collision(_body: Node) -> void:
	...
	can_play_hit = false
	var track := "hit" + str(randi_range(1, 6))
	sfx["parameters/switch_to_clip"] = track
	sfx.play()
	hit_timeout = get_tree().create_tween()
	hit_timeout.tween_callback(_sfx_hit_done).set_delay(0.25)
	# Try setting pitch_scale in a tween (didn't work)
	# https://github.com/godotengine/godot/issues/97704
	#get_tree().create_tween().tween_property(sfx, "pitch_scale", 2, 0.05)

func _sfx_hit_done() -> void:
	can_play_hit = true
	sfx.stop()
